...
from DataLoader import data_load_task_1_CE, data_load_task_2_LoS_NLoS, data_load_task_3_V_WL
from utils import *
...
def main():
    ...
    train_data, test_data = data_load_task_3_V_WL(args)  # 加载数据
    ...
    ).Train(train_data, args.epochs, seed=100, mask_ratio=0, mask_strategy='fre', task_id=3)
...
def freeze_wifo_except(model):
    ...
    # for param in model.blocks[-2:].parameters():
    #     param.requires_grad = True
    for param in model.Fine_Tune_Layer0_T3_VWL.parameters():
        param.requires_grad = True
    for param in model.Fine_Tune_Layer_T3_VWL.parameters():
        param.requires_grad = True
