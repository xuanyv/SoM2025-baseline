...
class WiFo(nn.Module):
    ...
    def __init__(self, patch_size=1, in_chans=1,
        ...
        # T1-CE fine-tune the decoder_pred
        self.Fine_Tune_Layer_T1_CE = nn.Sequential(
            nn.Linear(decoder_embed_dim, 2 * decoder_embed_dim),
            nn.ReLU(),
            nn.Linear(2 * decoder_embed_dim, self.t_patch_size * patch_size**2 * 2),
        )

        # T2-LoS-NLoS
        self.Fine_Tune_Layer0_T2_LoS_NLoS = nn.Sequential(
            nn.Conv1d(6*2*32, 32, 3, 1, 1),
            nn.ReLU(),
            nn.Conv1d(32, 1, 3, 1, 1),
        )
        self.Fine_Tune_Layer_T2_LoS_NLoS = nn.Sequential(
            nn.Linear(embed_dim, 2),
        )
        # T3-VWL
        self.Fine_Tune_Layer0_T3_VWL = nn.Sequential(
            nn.Conv1d(1*32*8, 32, 3, 1, 1),
            nn.ReLU(),
            nn.Conv1d(32, 1, 3, 1, 1),
        )
        self.Fine_Tune_Layer_T3_VWL = nn.Sequential(
        ...
        self.res18.eval()
        self.res18_align_layer = nn.Linear(1000, embed_dim)
    ...
    def forward(self, channel, rgb=None, mask_ratio=0.5, mask_strategy='random', seed=None, data='none', task_id=1):
        # channel: [B, 2, N, K]

        if len(channel.shape) == 4:
            channel = channel.unsqueeze(2).repeat(1, 1, 4, 1, 1)
        _ = self.patchify(channel)
        scale = [1,1,1]

        T, H, W = channel.shape[2:] # channel的维度为256*2*12*10*20(T*H*W)
        latent, mask, ids_restore, input_size = self.forward_encoder(channel, mask_ratio, mask_strategy, seed=seed, data=data, mode ='backward', scale=scale)
        if task_id == 1:
            pred = self.forward_decoder(latent, ids_restore, mask_strategy, input_size=input_size, data=data,
                                        scale=scale)  # [N, L, p*p*1]
            pred = self.Fine_Tune_Layer_T1_CE(pred)
            Len = self.t_patch_size * self.patch_size ** 2
            pred_complex = pred[:, :, :Len] + 1j * pred[:, :, Len:]
            return self.unpatchify(pred_complex)
        elif task_id == 2:
            # pred = torch.mean(latent, dim=1)  # B, d
            pred = self.Fine_Tune_Layer0_T2_LoS_NLoS(latent).squeeze(1)
            pred = self.Fine_Tune_Layer_T2_LoS_NLoS(pred)  # B, 2
            return pred
        elif task_id == 3:
            rgb_feature = self.res18_align_layer(self.res18(rgb))
            csi_feature = self.Fine_Tune_Layer0_T3_VWL(latent).squeeze(1)
            combine_feature = torch.cat([rgb_feature, csi_feature], dim=1)
            pred = self.Fine_Tune_Layer_T3_VWL(combine_feature)
            pred_x = torch.ones_like(pred) * -14.61
            pred = torch.cat([pred_x, pred], dim=1)
            return pred
